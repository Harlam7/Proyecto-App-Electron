...
btnActualizar.addEventListener('click', () => {
	...
	|| txtFechaE.value == '' )) {
		...
		matriculanoviembre: txtDiciembre.value,
...
const updateClient = (data) => {
    window.ipcRender.send('updateClient', data);
...
if (localStorage.getItem('reload') == '1') {
    ...
    window.ipcRender.invoke('confirmUpdateClient').then((confirm) => {
        ...
        if (confirm == 1) {
            ...
            swalWithBootstrapButtons.fire({
                title: '¡Error!',
                text: "La información no se cambia",
                icon: 'error',
                confirmButtonClass: 'mr-2'
            }).then((result) => {
                if (result.value) {
...
const mostrarClients4 = (clients) => {
    ...
    for (let i = 0; i < clients.length; i++) {
        texto +=
            ...
            <td class="text-center"><button type="button" class="btn btn-danger" onclick="showSwal('passing-parameter-execute-cancel', '${typeof clients[i].id === 'string' ? clients[i].id : ''}')">Eliminar</button></td>

            </tr>
        `;
    }
    
    //ESTA FUNCIÓN IBA ARRIBA   
