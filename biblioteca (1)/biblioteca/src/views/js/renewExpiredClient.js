const renewExpiredClient = (id) => {
    window.ipcRender.send('consultClient', id);

    location.href = './form-renewExpiredClient.html';
}
    ...
    for (let i = 0; i < clients.length; i++) {
        texto +=
            ...
            <td>${clients[i].id}</td>
                ...
                <td class="text-center"><button type="button" class="btn btn-success" onclick="renewExpiredClient('${clients[i].id}')">Renovar</button></td>
                <td class="text-center"><button type="button" class="btn btn-primary" onclick="sendEmail()">E. C.</button></td>
                </td>
            </tr>
        `;
    }
